...
def main(args):
    ...
    '''MODEL LOADING'''
    MODEL = importlib.import_module(f'models.{args.model}')     # models.pointnet2_seg 
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for i, (points, target) in tqdm(enumerate(train_loader), total=len(train_loader), smoothing=0.9):
            ...
            points, target = points.to(device), target.to(device).float()  # [B, N, 3], [B, N]
            points = points.transpose(2, 1)    # [B, N, 3]  --> [B, 3, N]
            ...
            loss = criterion(seg_pred, target, trans_feat, weights=None)
            loss.backward()
            optimizer.step()
