...
def main(args):
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for i, (points, target) in tqdm(enumerate(train_loader), total=len(train_loader), smoothing=0.9):
            ...
            seg_pred, trans_feat = model(points)   # seg_pred: [B, N, NUM_CLASSES]
        ...
        train_loss_list.append((loss_sum / num_batches).item())
        ...
        with torch.no_grad():
            ...
            val_loss_list.append((loss_sum / float(num_batches)).item())
            val_acc_list.append(total_correct / float(total_seen))
...
def save_metrics_as_csv(log_dir, train_loss, train_acc, val_loss, val_acc, val_miou):
    df = pd.DataFrame({
