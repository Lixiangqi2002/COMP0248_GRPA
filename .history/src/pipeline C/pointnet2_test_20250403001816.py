...
def main(args):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (points, labels) in tqdm(enumerate(test_loader), total=len(test_loader)):
            ...
            for _ in range(args.num_votes):
                ...
                for b in range(points.size(0)):
                    for n in range(points.size(1)):
                        if labels[b, n] != 0 and not np.isinf(labels[b, n]):
                            vote_label_pool[b, n, int(pred_choice[b * points.size(1) + n])] += 1
            ...
            labels = labels.view(-1, 1)[:, 0]           # [B*N] -> [B*N, 1]


            # loss = F.cross_entropy(seg_pred, labels)
            loss = criterion(seg_pred, labels.view(-1).long(), trans_feat=None, weight=None)

            pred_choice = seg_pred.cpu().data.max(1)[1].numpy()  # 得到每个点的预测类别
            correct = (pred_choice == labels.cpu().numpy()).sum().item()
            ...
            for l in range(NUM_CLASSES):
                ...
                total_correct_class_tmp[l] += np.sum((pred_choice == l) & (labels.cpu().numpy() == l))
                total_iou_deno_class_tmp[l] += np.sum(((pred_choice == l) | (labels.cpu().numpy() == l)))
