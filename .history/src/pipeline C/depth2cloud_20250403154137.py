...
class point_cloud_generator():
    ...
    def write_ply(self):
        ...
        for tag, df in zip(['full', 'mask'], [self.df_full, self.df_masked]):
            ...
            # e.g. out_file: data/frame00001_full.ply
            # e.g. out_file: data/frame00001_mask.ply
    ...
    def save_npy(self):
        ...
        pc = self.df_full.T[:, :6]  # shape (N, 6)
        mask_pc = self.df_masked.T[:, :3]  # shape (M, 3)
        
        pc_rounded = np.round(pc[:, :3], 3)
        ...
        merged = np.concatenate([pc, labels.reshape(-1, 1)], axis=1)  # shape (N, 7)
        np.save(f"{self.output_prefix}_merged.npy", merged)
    ...
    for split in all_splits:
        ...
        for scene in subfolders:
            ...
            for polygon_list, img_file, depth_file in zip(tabletop_labels, image_files, depth_files):
                ...
                # e.g. output_dir+f"/{dir_name}": data/dataset/point_clouds/train
    ...
    for root, _, files in os.walk(split_dir):
        for file in files:
            if file.endswith("_mask.npy"):
                ...
                merged_path = mask_path.replace('_mask.npy', '_merged.npy')

                pc = np.load(pc_path)  # (N, 6)
                mask = np.load(mask_path)  # (M, 3)

                pc_xyz = np.round(pc[:, :3], 3)
                mask_xyz = np.round(mask, 3)

                mask_set = set(map(tuple, mask_xyz))
                labels = np.array([1 if tuple(p) in mask_set else 0 for p in pc_xyz], dtype=np.int64)

                merged = np.concatenate([pc, labels[:, None]], axis=1)  # shape: (N, 7)
