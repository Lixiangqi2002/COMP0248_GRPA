...
class PointCloudDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # === Step 1: create 9D features ===
        # # Step 1.1: center point for local frame
        # center = np.mean(pc, axis=0)

        # # Step 1.2: fake RGB (0s), shape = (N, 3)
        # rgb = np.zeros_like(pc)

        # # Step 1.3: normalize global coords
        # pc_max = np.max(pc, axis=0) + 1e-6  # avoid division by zero
        # normalized_coords = pc / pc_max  # shape = (N, 3)

        # # Step 1.4: local centered coords
        # pc_local = pc - center  # shape = (N, 3)

        # # Step 1.5: concatenate -> (local_xyz, fake_rgb, normalized_xyz)
        # features = np.concatenate([pc_local, rgb, normalized_coords], axis=1)  # shape: (N, 9)

        return torch.tensor(pc, dtype=torch.float32), torch.tensor(label, dtype=torch.long), idx
