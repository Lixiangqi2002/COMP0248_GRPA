...
def main(args):
    ...
    weights = torch.Tensor(full_dataset.labelweights).cuda()
    
    # TRAIN_DATASET = PointCloudDataset(split='train', data_root=root, num_point=NUM_POINT, test_area=args.test_area, block_size=1.0, sample_rate=1.0, transform=None)
    # TEST_DATASET = PointCloudDataset(split='test', data_root=root, num_point=NUM_POINT, test_area=args.test_area, block_size=1.0, sample_rate=1.0, transform=None)
    ...
    '''MODEL LOADING'''
    MODEL = importlib.import_module(args.model)     # pointnet2_seg 
