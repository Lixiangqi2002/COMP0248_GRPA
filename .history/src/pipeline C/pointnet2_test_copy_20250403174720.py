...
def visualize_prediction_vs_gt(points_tensor, pred_labels, gt_labels, batch_index=0):
    """
    使用 Open3D 并排展示预测结果和 Ground Truth。
    
    参数:
        points_tensor: torch.Tensor, shape [B, N, 9]，输入点云
        pred_labels: np.ndarray, shape [B, N]，模型预测的结果（整型）
        gt_labels: torch.Tensor or np.ndarray, shape [B, N]，Ground Truth 标签
        batch_index: int, 默认显示第几个 batch 中的样本（默认0）
    """

    # 获取对应 batch 的数据
    points_np = points_tensor[batch_index].cpu().numpy()   # [N, 9]
    xyz = points_np[:, :3]

    # 转换 ground truth labels
    if isinstance(gt_labels, torch.Tensor):
        gt_labels_np = gt_labels[batch_index].cpu().numpy()
    else:
        gt_labels_np = gt_labels[batch_index]

    pred_labels_np = pred_labels[batch_index]  # already numpy

    # 定义颜色映射
    color_map = {
        0: [0.2, 0.4, 1.0],  # 非桌子 -> 蓝色
        1: [1.0, 0.2, 0.2],  # 桌子 -> 红色
    }

    # 构建预测点云
    pred_colors = np.array([color_map[int(label)] for label in pred_labels_np])
    pcd_pred = o3d.geometry.PointCloud()
    pcd_pred.points = o3d.utility.Vector3dVector(xyz)
    pcd_pred.colors = o3d.utility.Vector3dVector(pred_colors)

    # 构建 GT 点云
    gt_colors = np.array([color_map[int(label)] for label in gt_labels_np])
    pcd_gt = o3d.geometry.PointCloud()
    pcd_gt.points = o3d.utility.Vector3dVector(xyz + np.array([0.6, 0, 0]))  # 平移以并排显示
    pcd_gt.colors = o3d.utility.Vector3dVector(gt_colors)

    print("左边为预测，右边为Ground Truth")
    o3d.visualization.draw_geometries([pcd_pred, pcd_gt], window_name="Prediction vs Ground Truth")
    ...
    with torch.no_grad():
        ...
        for batch_idx, (points, labels) in tqdm(enumerate(test_loader), total=len(test_loader)):
            ...
            for _ in range(args.num_votes):
                ...
                # find the predicted class for each point
                pred_choice = seg_pred.cpu().data.max(1)[1].numpy()  # [B*N]

                # visualize the prediction
                print(f"Shape of points: {points.shape}")
                print(f"Shape of pred_choice: {pred_choice.shape}")
                pred_choice_reshaped = pred_choice.reshape(points.size(0), points.size(2))  # [B, N]
                # print(f"Shape of pred_choice_reshaped: {pred_choice_reshaped.shape}")
                # print(f"Shape of labels: {labels.shape}")
                # visualize_prediction_vs_gt(points, pred_choice_reshaped, labels)
