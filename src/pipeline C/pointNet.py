...
class PointNetSegmentation(nn.Module):
    def __init__(self):
        ...
        self.segmentation_head = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(0.3),
            nn.Conv1d(128, 2, 1)     # output per-point classification (B, 2, N)
        )
...
def train(train_loader, val_loader, device):
    ...
    for epoch in range(100):
        ...
        for pc, label in train_loader:
            pc, label = pc.to(device), label.to(device).float()  # [B, N, 3], [B, N]
            out = model(pc)  # out: [B, N, 2]

            loss = F.cross_entropy(out.reshape(-1, 2), label.reshape(-1).long())
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
        ...
        with torch.no_grad():
            for pc, label in val_loader:
                ...
                # out.view(-1, 2): [B*N, 2], label.view(-1).long(): [B*N]
                loss = F.cross_entropy(out.reshape(-1,2), label.reshape(-1).long())
                val_total_loss += loss.item() * pc.size(0)
                
                pred = out.argmax(dim=2)  # out: [B, N, 2] -> pred: [B, N]
